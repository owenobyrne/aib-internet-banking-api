...
public class StatementPage extends FSSUserAgent
{
        ...
        throws Exception
    {
        ...
        if(inputParams.get("index") != null)
        {
            ...
            p = Pattern.compile("action=\"statement.htm\" method=\"POST\" onsubmit=\"return isClickEnabled\\(\\)\">\\s*<div>\\s*<label>Change account:\\s*<select id=\"index\" name=\"index\">\\s*(.*?)\\s*</select>\\s*</label>\\s*<input type=\"hidden\" name=\"transactionToken\" id=\"transactionToken\" value=\"(\\d+)\"/>");
            m = p.matcher(page);
            if(m.find())
        ...
        } else
        {
            ...
            p = Pattern.compile("action=\"statement.htm\" method=\"post\"><input type=\"hidden\" name=\"isFormButtonClicked\" value=\"false\" /><input type=\"hidden\" name=\"transactionToken\" id=\"transactionToken\" value=\"(\\d+)\"/>");
        ...
        for(; m.find(); addl.addAccountDropdownItem(new AccountDropdownItem(m.group(1), m.group(2), "")))
            Log.info((new StringBuilder()).append("Account name - ").append(m.group(2)).toString());

        outputParams.put("accounts", addl);
        p = Pattern.compile("<tr class=\"j*ext01\">\\s*<td>(\\d\\d)/(\\d\\d)/(\\d\\d)</td>\\s*<td>([^<]*)</td>\\s*<td class=\"aibTextStyle10\">([^<]*)</td>\\s*<td class=\"aibTextStyle10\">([^<]*)</td>\\s*<td class=\"aibTextStyle10\">([^<]*)</td></tr>");
        m = p.matcher(page);
        ...
        while(m.find()) 
            ...
            {
                t = new Transaction(new GregorianCalendar(2000 + Integer.parseInt(m.group(3)), Integer.parseInt(m.group(2)) - 1, Integer.parseInt(m.group(1))), "New Interest Rate", m.group(5), m.group(6), m.group(7));
                transactions.addTransaction(t);
            } else
            if(t != null && "".equals(m.group(5)) && "".equals(m.group(6)))
            {
                t.addSubNarrative(m.group(4));
                transactions.replaceLastTransaction(t);
            } else
            {
                t = new Transaction(new GregorianCalendar(2000 + Integer.parseInt(m.group(3)), Integer.parseInt(m.group(2)) - 1, Integer.parseInt(m.group(1))), m.group(4), m.group(5), m.group(6), m.group(7));
                transactions.addTransaction(t);
            }
